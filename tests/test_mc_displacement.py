...
def fixture_mc_displacement_ingaas():
    ...
    nd = MCDisplacement(
        ...
        max_angle_deg=180.,
        unit_cell=unit_cell,
        displace_func=lambda pos, uc: MCDisplacement.displace_gaussian(
    ...
    for i, uc_pos in enumerate(mc_displacement_ingaas._unit_cell_pos):
        for j, atom in enumerate(mc_displacement_ingaas._atomic_numbers_vars[uc_choices[i]]):
            ...
            atoms.append(Atom(atom, displacement +
                              uc_pos + mc_displacement_ingaas._atom_pos_in_uc_orig[j]))
...
def test_displace_gaussian_normal_operation():
    ...
    displaced = MCDisplacement.displace_gaussian(
        positions=positions,
...
def test_random_occupation_matches_zero_displacement(
    ...
    mc_random_gaas._rng = np.random.default_rng(2)
    mc_displacement_ingaas._rng = np.random.default_rng(2)

    mc_random_gaas.setup_spherical_crystal(2)
    ...
    mc_displacement_ingaas.setup_spherical_crystal(2)
    mc_displacement_ingaas._displace_func = lambda pos, uc: (
        MCDisplacement.displace_gaussian(pos, uc, sigma=0., atoms_to_displace=[]))
